...
export class SzCrossSourceResultsDataTable extends SzDataTable implements OnInit, OnDestroy {
    ...
    */
     ...
     * @internal
     */
    private _visibilitySelectableColumnsAsMap: Map<string,string> = new Map();

    // ----------------------------------- child/dom refs ------------------------------------
    /** When the overlay for the context menu is active the ref is stored here
     * @internal
    */
    private contextMenuOverRef: OverlayRef | null;
    ...
    */
    ...
    public get visibilitySelectableColumns(): Map<string,string> {
      return this._visibilitySelectableColumnsAsMap;
    }
    /*
    ...
    */
        ...
        .pipe(
          filter(event => {
            ...
            if(this._contextMenuShowing) {
              ...
              return !!this.contextMenuOverRef && !this.contextMenuOverRef.overlayElement.contains(clickTarget);
    ...
    private generateSelectableColumnsMap() {
      ...
      this._visibilitySelectableColumnsAsMap = _vCols;
    }
    /** get the css `order` value that the cell should slot itself in to.
     * @internal
    */
    getCellOrder(columnName: string, rowsPreceding: number) {
    ...
    /** whether or not there are additional rows for a result that are not part of the
    ...
    */
    ...
    override moveColumn(fieldName: string, orderModifier: number) {
      ...
      //} else if(shiftRight) {
        ...
        // first col is always entityId so real jump to front starts
    ...
    */
    ...
    // ------------------------------------ event handlers -----------------------------------
    /** when a cell is clicked on we detect whether or not it's an "entityId" cell and if so
    ...
    */
    private onDocumentClick(event: MouseEvent) {
      ...
      if(this.contextMenuOverRef) {
        this.contextMenuOverRef.dispose();
        this.contextMenuOverRef    = undefined;
    ...
    public onHeaderContextMenu(event: MouseEvent, col: any) {
      ...
      this.contextMenuOverRef = this.overlay.create({
        positionStrategy,
        scrollStrategy: this.overlay.scrollStrategies.close()
      });

      this.contextMenuOverRef.attach(new TemplatePortal(this.sz_dt_header_context_menu, this.viewContainerRef, {
        $implicit: col
      }));
    }
